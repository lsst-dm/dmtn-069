...
depicts the theoretical run of false positives with detection
threshold, using the mean seeing for each sample.  We find that, in
general, our results are consistent with this theoretical limit.  One
noteworthy exception is in the deconvolution configuration, where the
0.88'' template is differenced against the 0.6'' science data without
prefiltering (leftmost column of left panel).  In this case, our
realized false positive rate was significantly {\it lower} than
expected from theory.  This is consistent with the results from W13
(Section 6.4), where deconvolution significantly enhanced the noise
properties of the difference image.
...
{\it underestimates} the empirical variance by 4--5\%.  Both of these
conclusions are consistent with the {\tt prefiltering = False} panels
in Figure~\ref{fig:fptheory}, where the false positives rates are
...
Finally, we designed a set of simulations that are expected to show
the effects of DCR.  This included 5 observations tracking the single
star field throughout a single night of observations.  The salient
...
\item We validate the amplitude and orientation of the DCR effect in
  ...
  {\tt M2.0V} stars is under-realized by $\sim 30\%$ compared to
  expectations.
...
\begin{figure}[!ht]
  ...
  \caption{{\bf Subtask 2: False Positive Rates vs. Theory}: Using the
    ...
    to a template devonvolution, are lower than expected due to an
    overestiamte of the noise by the variance plane.  The
...
\caption[]{Summary of the results of subtask 2.  We show the average
  ...
  than 1 in all cases.  Comparisons of these numbers with theorecial
...
\caption[So I can have 2 paragraphs]{The expected and measured
  ...
  quantitatively similar results.  We also see no dependce of these
