...
suite of 15s $i$--band images taken at zenith angle of $20.2^{\circ}$
and in seeings of 0.66, 0.8, and 1.2 arcseconds.  These were
differenced against a 300s $i$--band image generated at the same
...
and how pre--filtering of the science {\tt Exposures} with their {\tt
  Psfs} affects this rate.  We limited this reanalysis to the 9 sensors
of central raft {\tt 2,2}.  The second subtask involved the generation
of $g$--band and $r$--band images in the same observing configuration,
to verify there is not wavelength dependence to our results.  For
these first and second subtasks, the simulated catalog included stars
of a single spectral energy distribution (SED) covering a narrow range
in (high S/N) brightness.  We find that our results are quantitatively
...
the angle that refraction displaces objects within image, which will
...
would have proceeded using script {\tt processCcd.py}.
...
similar S/N distribution as in subtask 1.  Once this was validated to
work, we ran two additional runs in the $g$ and $r$--bands.  The
numbers of false positives are reported in Table~\ref{tab:task2}.  We
average over all 9 CCDs in the raft to report the mean numbers of
false positives per CCD.  

{\bf TODO} Show figure with theory.  

{\bf TODO} We note that the numbers of false positives are
consistently {\it underestimated} in the deconvolution configuration.
We examine the noise properties of the resulting images using script
{\tt python/examineVariance.py}.
...
\item The treatment of DCR will likely be a complicated one, and at
  ...
  on this effect, as the template images need to first be coadded
  (which requires registration of input images {\it and} of the
  objects within the images) before being warped to the image
  coordinate system to be used for subtraction.
...
where {\tt -w} defins the work directory (which must already exist),
...
A calibraton file must be created for {\tt astrometry.net}, which is
...
naming convention defined by envorinment variable {\tt P} above:
{\small
\begin{Verbatim}[frame=single]
