...
\date{\today}
\author{
  %Andrew Becker (UW) {\it for LSST Data Management}
}
\begin{document}
\maketitle
...
from W13 (which used {\tt phoSim} v3.2.2), but were able to create
qualitatively similar images using {\tt phoSim} v3.3.2 and the W13
control files, with the additional control parameter {\tt
  fieldanisotropy 0}.  We note that the realized seeing values of the
images, as measured by the adaptive second moments of the {\tt Psf},
are systematically ($\sim 5\%$) smaller in the newer data.  This was
...
prefiltering (leftmost column of top panel).  In this case, our
