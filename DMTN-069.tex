...
\setDocChangeRecord{%
\addtohist{1.0}{2014-04-04}{Version 1 of document}{A.~Becker}
\addtohist{1.05}{2014-05-30}{Released version.}{A.~Becker}
\addtohist{}{2017-12-29}{Converted to DMTN-069.}{T.~Jenness}
...
The underlying image simulation software (\texttt{phoSim}) and the DM
stack have evolved since our W13 work, \texttt{phoSim} from v3.2.2 to
...
pre--filtering of the science \texttt{Exposures} with their \texttt{Psfs}
affects this rate.  We limited our reanalysis to the central sensor of
central raft \texttt{2,2}, and quantitatively (but not exactly) recover
...
After generation we ran the simulated \texttt{eImages} through single
frame measurement (SFM) using the script \texttt{processEimage.py}.  Note
that we did \textit{not} use the raw per--amplifier simulated images,
instead choosing to operate on the \texttt{eImages} because the former
...
would have proceeded using the script \texttt{processCcd.py}.
...
class \texttt{Winter2013ImageDifferenceTask} (so--called as it was
...
second pre--filtering the science image with its Psf\footnote{Briefly,
  ...
  more details please see: \citeds{LDM-227}.}.
We next ran detection and measurement on each difference image to
extract the \texttt{DiaSources} from the images.  We looked at detections
...
and what was expected based on the inputs to \texttt{phoSim}.  The
...
In the following, we reference the 3 bins in seeing as \texttt{Seeings
  1,2,3} for seeing = 0.6'', 0.88'', 1.2'', respectively, and the 5
visits at different airmasses as \texttt{Visits A,B,C,D,E}.  We briefly
...
W13, we first validated that the \texttt{phoSim} software and DM stack
(and their interaction) had not significantly changed in the interim.
We note that W13 used \texttt{phoSim} version v3.2.2 and DM stack v7\_2;
W14 uses v3.3.2 and v7\_3, respectively.  Starting with the W13 \texttt{
...
central sensor \texttt{1,1} of the central raft \texttt{2,2}.  We performed
...
The \texttt{phoSim} configuration files for this subtask are contained in
subdirectory \texttt{sims3/} of this repository.  Work on this subtask
...
range in color.  We chose a \texttt{G0V} star to be our reference source,
and replaced 80\% of the source SEDs with those of SED \texttt{
  km20\_6000.fits\_g30\_6020.gz}.  When performing the image
differencing, \textit{only} these objects were used for the creation of
...
of the SEDs with those of a ``blue'' source -- spectrum \texttt{
  kp01\_9750.fits\_g45\_9830.gz} representing an \texttt{A0V} star -- and
10\% of the SEDs with those of a ``red'' source -- spectrum \texttt{
  m2.0Full.dat.gz} representing an \texttt{M2.0V} star.  We modified the
...
similar quality as in subtask 1 above.  The \texttt{phoSim} configuration
files for this are contained in subdirectory \texttt{sims5/} of this
repository.

We further generated sets of simulations in the $g$--band and
$r$--band using the exact same set of trim files except for the
requested \texttt{Opsim\_filter}.  Analysis of these images showed no
...
described in Section~\ref{sec:task2} below.  The \texttt{phoSim}
configuration files for this are contained in subdirectory \texttt{
...
\caption[So I can have 2 paragraphs]{Summary of the configuration
  ...
  the template image (\texttt{Opsim\_rawseeing} = 0.88'' and \texttt{
    SIM\_VISTIME} = 300) and then 3 bins in seeing for the science
  image corresponding to \texttt{Opsim\_rawseeing} = 0.6'', 0.88'', and
  1.2'' for seeing bins \texttt{1,2,3}, respectively.  All science images
  were simulated with \texttt{SIM\_VISTIME} = 15.  Overall, this yielded
  $5 \times 3 \times 4 = 60$ runs of \texttt{phoSim}, which are encoded
  as \texttt{visitId=X00Y00Z} where \texttt{X} represents the filterId
  $gri$=\texttt{123}, \texttt{Y} represents the visit \A\B\C\D\E = \texttt{
    01234}, and \texttt{Z} represents the seeing: \texttt{0} for the
  template and \texttt{123} for the science images.  All 9 CCDs within
  \texttt{raft=2,2} were simulated.  }
\label{tab:visits}
\end{table}

Starting with the \texttt{sims5/} trim files referenced above, we further
...
The \texttt{phoSim} configuration files for this are contained in
subdirectory \texttt{sims8/} of this repository.  Work on this subtask
...
\begin{eqnarray}
R(\lambda, Z) & = & \frac{n(\lambda)^2 - 1}{2~n(\lambda)^2}~\mathrm{tan}(Z)
\end{eqnarray}
...
\begin{figure}[!t]
  ...
  \caption{\textbf{Effective Spectral Energy Distributions} : The
    effective spectra of 5 reference objects -- a ``blue'' \texttt{A0V},
    a reference \texttt{G0V}, and a ``red'' \texttt{M2.0V} star, along with
    ...
    subpanels, the x--axis is wavelength.  The \texttt{A0V}, \texttt{G0V},
    and \texttt{M2.0V} spectra correspond to {CAT\_SHARE\_DATA} files
    \texttt{kp01\_9750.fits\_g45\_9830.gz}, \texttt{
      km20\_6000.fits\_g30\_6020.gz}, and \texttt{m2.0Full.dat.gz}
...
\begin{figure}[!t]
  ...
  \caption{\textbf{Refraction Amplitude vs. Filter and Spectral Energy
    ...
    arcminute.  This figure may be recreated using the script \texttt{
      python/DCR.py}.}
  \label{fig:refraction}
\end{figure}

We examined the theoretical amplitude of this effect using discrete
spectra from the \texttt{phoSim} source catalog.  We used the same
...
sources.  Left to right, these represent a ``blue'' \texttt{A0V} star, a
reference \texttt{G0V} star, a red \texttt{M2.0V} star, and an active
...
show the \textit{differential} refraction of each spectrum, DCR, with
respect to the \texttt{G0V} star.  Note that the maximum amplitude of DCR
...
using the \texttt{throughputs} package to yield the flux--weighted DCR
...
%from W13 using the version of \texttt{phoSim} that was used at the time
...
%up-rev'd to the current v3.3.2 of the \texttt{phoSim} software.  Since
%additional features have been added to \texttt{phoSim}, and many are
%turned \textbf{on} by default, we were again unable to produce an
%\textit{exact} match to the W13 images.  However, we were able to create
%qualitatively similar images by using \texttt{phoSim} v3.3.2 and the W13
%control files, with the additional control parameter \texttt{
%  fieldanisotropy 0}.  We note that the realized seeing values of the
%images, as measured by the adaptive second moments of the \texttt{Psf},
...
\begin{tabular}{ccccc}
\hline
\multicolumn{5}{|c|}{Subtask 1: False Positives in \texttt{raft=2,2 sensor=1,1}} \\ \hline \\
Seeing  & Prefilter    & W13  & W14 pipeline on W13 data   & W14   \\
\hline
\texttt{1} & True         & 10   & 10                         & 17    \\
        & False        & 23   & 19                         & 68630 \\
\texttt{2} & True         & 7    & 6                          & 6     \\
        & False        & 8    & 8                          & 6     \\
\texttt{3} & True         & 3    & 3                          & 3     \\
...
\caption[]{Summary of the results of subtask 1, which aims to
  reproduce the results of W13 using \texttt{phoSim} v3.3.2 and the
  current v7\_3 stack.  The first column shows the numbers of false
  positives from \texttt{raft=2,2 sensor=1,1} of the original W13
  ...
  in the deconvolution configuration (seeing \texttt{1}, prefilter False)
  were triggered by the bug resolved in ticket \#3143.  Results from
  this table were generated using script \texttt{
    python/countDiaSourcesT1.py} in this repository.}
\label{tab:task1}
\end{table}

We were unable to reproduce an \textit{exact} match to simulated images
from W13, but were able to create qualitatively similar images using
\texttt{phoSim} v3.3.2 and the W13 control files, with the additional
control parameter \texttt{fieldanisotropy 0}.  We note that the realized
seeing values of the images, as measured by the adaptive second
moments of the \texttt{Psf}, are systematically ($\sim 5\%$) smaller in
...
the algorithms.  The W14 analysis is qualitatively similar,
\textit{except} the configuration that leads to deconvolution of the
template (seeing visit \texttt{1} and no prefiltering).  The moderately
...
\begin{tabular}{cccc}
\hline
\multicolumn{4}{|c|}{Subtask 2: Average False Positives in \texttt{raft=2,2}} \\ \hline \\
Seeing   & Filter & N & N  \\
...
\caption[]{Summary of the results of subtask 2.  We show the average
  (over all 9 sensors of raft \texttt{2,2}) of the total number of false
  ...
  from this table were generated using script \texttt{
...
\begin{figure}[!h]
  \centering
  \subfloat[Prefiltering = False]{{\includegraphics[width=0.65\textwidth]{outputs5b_doPreConvolveFalse_fp.png} }} \\
  \subfloat[Prefiltering = True]{{\includegraphics[width=0.65\textwidth]{outputs5b_doPreConvolveTrue_fp.png} }} \\
  \caption{\textbf{Subtask 2: False Positive Rates vs. Theory}: Using the
    mean realized seeing for each passband and \texttt{Opsim\_rawseeing}
    combination, we plot the expected numbers of false positives per
    sensor (both positive and negative going) as a function of
    detection threshold.  We plot the median number of \texttt{
      DiaSources} detected at 5 sigma across the nine \texttt{raft=2,2}
    CCDs in red, using the interquartile range to calculate the RMS.
    Results using postfiltering of the data with its \texttt{Psf} are on
    the top, and prefiltering are on the bottom.  Note that the
    postfiltering false positives in seeing \texttt{1} which correspond
    to a template deconvolution, are lower than expected due to an
    overestimate of the noise by the variance plane.  The
    postfiltering positives for seeing \texttt{2,3} are consistently
    ...
    curves.  This figure was created using the script \texttt{
...
\texttt{DiaSources} in red.  The black line depicts the theoretical run
...
significantly \textit{lower} than expected from theory.  This is
...
postfiltering data, \textit{including the deconvolution configuration}.
...
Secondly, we took the difference images that had \textit{not} been
filtered by their \texttt{Psf}, and perform the \texttt{Psf} convolution.
...
deconvolution configurations, the variance plane provided an
\textit{overestimate} of the empirical variance by 30--40\% ($V_e$/$V_p$ =
0.7--0.76).  For the convolution configurations, the variance plane
\textit{underestimates} the empirical variance by 4--5\%.  An
...
results.  Figure~\ref{fig:fptheory} shows that the \texttt{
  prefilter=False} false positives rates are significantly lower than
theory in the prefiltering seeing \texttt{1} data (deconvolution; the
variance plane overestimates the true noise) and moderately but
consistently higher than theory in the seeing \texttt{2,3} data
...
used script \texttt{python/examineVariance.py} for this analysis.
...
\begin{figure}[!ht]
  ...
  \caption{\textbf{Designed Orientation of DCR in W14 PhoSim Runs}: This
    figure represents the anticipated orientations of DCR in the W14
    \texttt{phoSim} data.  The x,y coordinate system is depicted, as well
    as the convention that angles (\texttt{rotTelPos}, \texttt{rotSkyPos})
    ...
    taken at zenith.  This figure was created using the script \texttt{
...
zenith and thus no DCR is expected).  These are set by \texttt{phoSim}
configuration parameter \texttt{rotTelPos}.  The direction to the pole is
also indicated, and set by \texttt{phoSim} configuration parameter \texttt{
  rotSkyPos}.  For reference, \texttt{rottelpos} is defined as (\texttt{
...
w.r.t. the \texttt{G0V} stars, are given in the first numerical column of
Table~\ref{tab:dcramp}.
...
\begin{figure}
    \centering
    \subfloat[Visit A]{{\includegraphics[width=0.46\textwidth]{outputs8bCA_dcr_wcs.png} }}
    \qquad
    \subfloat[Visit B]{{\includegraphics[width=0.46\textwidth]{outputs8bCB_dcr_wcs.png} }} \\
    \subfloat[Visit D]{{\includegraphics[width=0.46\textwidth]{outputs8bCD_dcr_wcs.png} }}
    \qquad
    \subfloat[Visit E]{{\includegraphics[width=0.46\textwidth]{outputs8bCE_dcr_wcs.png} }} \\
    \caption{\textbf{Wcs-Derived Orientations of PhoSim Data}: These
      figures show the orientations of the Right Ascension and
      Declination axes (red), and Azimuth and Altitude axes (blue) of
      visits \A,\B,\D,\E.  Arrows represent the directions of
      \textit{increasing} coordinate value.  The Ra,Dec axes are the same in
      all images since they were designed to have a common \texttt{
        rotTelPos}.  Ideally, the directions of increasing Alt will
      correspond to the \texttt{rotSkyPos} depicted in
      Figure~\ref{fig:phosimdcr}.  All coordinate system orientations
      were derived from the fitted Wcs of the \texttt{calexp} of the
      $g$--band observation of seeing \texttt{2} data.  To determine the
      orientations empirically, small steps were taken in each
      coordinate starting at the center of the image, and the \texttt{
        Wcs} and topocentric corrections used to map these back into
      offsets in the pixel plane.  This figure was created using the
      script \texttt{python/compareDcrFromSims.py}.}
    \label{fig:wcsdcr}
\end{figure}

Our first validation of DCR comes from looking at the fitted \texttt{Wcs}
of each \texttt{calexp} to establish directionality.  Using a topocentric
...
similar to the requested \texttt{rotTelPos}, typically to within a
degree.
...
Visit    & SED & \texttt{rotTelPos} & \texttt{Wcs} + Topo & Measured \\
         &     &                 &                  & Dipole Orientation \\
\hline
\A & \texttt{A0V}   & $145.7^{\circ}$ & $144.9^{\circ}$ & $145.7^{\circ}$    \\
   & \texttt{M2.0V} & $325.7^{\circ}$ & $324.9^{\circ}$ & $325.6^{\circ}$    \\
\hline
\B & \texttt{A0V}   & $152.3^{\circ}$ & $151.1^{\circ}$ & $151.7^{\circ}$    \\
   & \texttt{M2.0V} & $332.3^{\circ}$ & $331.1^{\circ}$ & $331.8^{\circ}$    \\
\hline
\D & \texttt{A0V}   & $349.9^{\circ}$ & $351.0^{\circ}$ & $349.3^{\circ}$    \\
   & \texttt{M2.0V} & $169.9^{\circ}$ & $171.0^{\circ}$ & $170.0^{\circ}$    \\
\hline
\E & \texttt{A0V}   & $356.4^{\circ}$ & $357.1^{\circ}$ & $355.2^{\circ}$    \\
   & \texttt{M2.0V} & $176.4^{\circ}$ & $177.1^{\circ}$ & $176.2^{\circ}$    \\
\end{tabular}
\caption[So I can have 2 paragraphs]{The expected and measured
  ...
  against the template taken at zenith (visit \C).  For the \texttt{
    rotTelPos} and \texttt{Wcs} columns, the numbers for the red \texttt{
    M2.0V} stars are simply $180^{\circ}$ from those of the blue \texttt{
    A0V} stars.  For the orientation column, we use fitted values from
  $g$--band data, seeing \texttt{1} (dipole results in the $r$--band show
  ...
  correctly.  Full results may be found in the file \texttt{NOTES}.}
\label{tab:dcrang}
\end{table}
...
\caption[So I can have 2 paragraphs]{The expected and measured
  ...
  respect to the positions of the reference \texttt{G0V} stars, which
  ...
  of $P$ = 520 mmHg, and ground temperature of $T=20$ C.  The \texttt{
  ...
  reference solution, determined using the \texttt{Wcs} and \texttt{Source}
  products.  Results in all passbands are consistent with zero
  refraction for the \C\C\ visits (within 0.001'').  We find moderate
  airmass dependence of the \texttt{G0V} star residuals with an amplitude
  ...
  bins \texttt{1,2,3}.  Theory results were determined using script \texttt{
    python/DCR2.py}.  Astrometric residuals were determined using
  script \texttt{astrometricResids.py}.  Full results may be found in the
  file \texttt{NOTES}.}
\label{tab:dcramp}
\end{table}
...
fitting for the \texttt{Wcs} model.  We grouped the stars into 3 bins
based upon their SED.  We then calculated the mean amplitude of
residuals to the astrometric fit, which is driven by the \texttt{G0V}
stars.  The second numerical column of Table~\ref{tab:dcramp} provides
this information.  We note that the DCR amplitudes for the \texttt{A0V}
objects are roughly consistent with theory, although uniformly
over--refracted by $\sim 4\%$.  The amplitudes of the relative
refraction of the \texttt{M2.0V} objects are under--refracted but by
...
the most dipoles in the $g$--band seeing \texttt{1} data, we used these
...
from the amplitudes suggested by the \texttt{Wcs} residuals.
...
\begin{figure}[!ht]
  ...
  \caption{\textbf{Number of False Positives, Postfiltering}: This figure
    shows the median number of false positives across the nine \texttt{
    ...
    sensitive to DCR.  This figure was created using the script \texttt{
      python/heatMap.py}.}
  \label{fig:heatpost}
\end{figure}
...
\begin{figure}[!ht]
  ...
  \caption{\textbf{Number of False Positives, Prefiltering}: Same as
    ...
    This figure was created using the script \texttt{python/heatMap.py}.}
  \label{fig:heatpre}
\end{figure}

Figures~\ref{fig:heatpost} and \ref{fig:heatpre} show the median
number of false positives per CCD within raft \texttt{2,2}.  These are
...
Figure~\ref{fig:fpregress}.  Data from the \texttt{A0V} stars are plotted
in blue, and from the \texttt{M2.0V} stars in red.  We use information
from all passbands and all seeings on this plot; the seeing \texttt{
...
\texttt{A0V}   & \mathrm{Prefilter=True}  & log_{10}~y = 4.52 + 2.25 ~log_{10}~x \\ \nonumber
            & \mathrm{Prefilter=False} & log_{10}~y = 5.53 + 2.75 ~log_{10}~x \\ \nonumber
%M-star True:  logy = 4.5773 + 2.6279 logx.  logy=0 at x=0.0181
%M-star False: logy = 4.7113 + 2.6713 logx.  logy=0 at x=0.0172
\texttt{M2.0V} & \mathrm{Prefilter=True}  & log_{10}~y = 4.58 + 2.63 ~log_{10}~x \\ \nonumber
            & \mathrm{Prefilter=False} & log_{10}~y = 4.71 + 2.67 ~log_{10}~x
\end{eqnarray}
...
the spectral type in the amplitude of the DCR shift, along the \texttt{
...
curves cross \texttt{y=1}, which effectively represents a doubling of the
...
value, amplitude/FWHM = 0.01 and 0.02 for the \texttt{A0V} and \texttt{
...
each image, which typically number 80--90 for the \texttt{A0V} and \texttt{
...
\texttt{A0V}   & \mathrm{Prefilter=True}  & log_{10}~y = 2.72 + 2.01 ~log_{10}~x \\ \nonumber
            & \mathrm{Prefilter=False} & log_{10}~y = 4.07 + 2.73 ~log_{10}~x \\ \nonumber
%M-star True:  logy = 3.0538 + 2.5494 logx.  logy=-2 at x=0.0104
%M-star False: logy = 3.1761 + 2.5848 logx.  logy=-2 at x=0.0099
\texttt{M2.0V} & \mathrm{Prefilter=True}  & log_{10}~y = 3.05 + 2.55 ~log_{10}~x \\ \nonumber
            & \mathrm{Prefilter=False} & log_{10}~y = 3.18 + 2.58 ~log_{10}~x
\end{eqnarray}
...
mark at $x \sim 0.005, 0.01$ for the \texttt{A0V} and \texttt{M2.0V} stars,
...
\textit{and} orientation of DCR at a range of other airmasses from
...
function of airmass along the \texttt{y}--axis, and difference in the
parallactic angle along the \texttt{x}--axis.  We overplot lines of
...
of dipoles in 0.6'' seeing for the \texttt{A0V} and \texttt{M2.0V} stars,
...
\textit{all} orientations yield similar results, and there is only a rate
...
5 degrees in orientation), nearly \textit{all} $g$--band data will show
...
\begin{figure}[!ht]
  \centering
  \subfloat[Prefiltering = False]{{\includegraphics[width=0.65\textwidth]{regressFalse.png} }} \\
  \subfloat[Prefiltering = True]{{\includegraphics[width=0.65\textwidth]{regressTrue.png} }} \\
  \caption{\textbf{DCR Impact on False Positives}: We plot the number of
    false positives that associate with \texttt{A0V} stars as the blue
    points, and \texttt{M2.0V} stars as the red points.  We normalize by
    the number expected due to fluctuations in the background, such
    that \texttt{y=1} (horizontal black line) represents a doubling of
    ...
    the smaller \texttt{x}--values, and the $g$--band data larger values.
    We additionally include information from all 3 seeings by plotting
    seeing \texttt{1} data as \textit{circles}, seeing \texttt{2} as
    \textit{squares}, and seeing \texttt{3} as \textit{triangles}.  We plot
    regressions of the form $log_{10}~y = a + b * log_{10}~x$ as solid
    lines, and the positions where these curves cross \texttt{y=1} as
    vertical dashed lines.  This figure was created using the script
    \texttt{python/fpRegressions.py}.}
  \label{fig:fpregress}
\end{figure}

\begin{figure}[!ht]
  \centering
  \subfloat[Prefiltering = False]{{\includegraphics[width=0.65\textwidth]{regressFalse2.png} }} \\
  \subfloat[Prefiltering = True]{{\includegraphics[width=0.65\textwidth]{regressTrue2.png} }} \\
  \caption{\textbf{DCR Impact on False Positives}: Same as
    Figure~\ref{fig:fpregress}, but the \texttt{y}--axis value represents
    ...
    was created using the script \texttt{python/fpRegressions2.py}.}
...
\begin{figure}[!ht]
  ...
  \caption{\textbf{Geometric Basis for Figure~\ref{fig:fpsummary}}: This
    ...
    amplitude for the basis of this study, labeled here as \texttt{DCR1}
    and representing the DCR of the \texttt{M2.0V} star at 1.25 airmasses
    with respect to the \texttt{G0V} star.  A second observation of this
    object is labeled \texttt{DCR2}, and is observed at a different
    airmass (representing a different DCR amplitude, plotted along the
    \texttt{y}--axis in Figure~\ref{fig:fpsummary}) and parallactic angle
    (denoted by theta, plotted along the \texttt{x}--axis in
    ...
    effectively the distance from point \texttt{DCR1} to the perimeter of
    the circle whose radius is \texttt{DCR2}.}
  \label{fig:fpgeom}
\end{figure}

\begin{figure}[!ht]
  \centering
  \subfloat[$g$--band, 1.0 airmass template]{{\includegraphics[width=0.29\textwidth]{dcrG1.png} }}
  \qquad
  \subfloat[$r$--band, 1.0 airmass template]{{\includegraphics[width=0.29\textwidth]{dcrR1.png} }}
  \qquad
  \subfloat[$i$--band, 1.0 airmass template]{{\includegraphics[width=0.29\textwidth]{dcrI1.png} }} \\
  \subfloat[$g$--band, 1.25 airmass template]{{\includegraphics[width=0.29\textwidth]{dcrG125.png} }}
  \qquad
  \subfloat[$r$--band, 1.25 airmass template]{{\includegraphics[width=0.29\textwidth]{dcrR125.png} }}
  \qquad
  \subfloat[$i$--band, 1.25 airmass template]{{\includegraphics[width=0.29\textwidth]{dcrI125.png} }} \\
  \caption{\textbf{DCR Offsets vs. Airmass and Orientation}: Each of
    ...
    row, 1.25 bottom row) and spectral type (\texttt{A0V} star top
    subpanel in each, \texttt{M2.0V} star bottom subpanel).  We calculate
    the DCR w.r.t. the \texttt{G0V} star at each reference airmass, and
...
%
    This figure was created using the script \texttt{python/DCR3.py}.}
  \label{fig:fpsummary}
\end{figure}
...
to know about DCR include the \texttt{WCS} class, for astrometric
modeling, and the \texttt{Psf} class, for its contribution to the
...
shifting of the image, but the shifting of every object \textit{within}
...
the \texttt{Wcs} and a putative \texttt{Dcr} class to map the location of an
...
must be treated as a bundle, to preserve the \texttt{Psf}.  It is unclear
...
during \texttt{Psf}/\texttt{Psf}--matching kernel modeling.  We can take
...
\item We are able to qualitatively reproduce the results of W13,
  ...
  Quantitative differences reflect changes in both \texttt{phoSim} and
...
\item However, the convolution of an unfiltered difference image with
  its \texttt{Psf} (i.e. the postfiltering process immediately preceding
  ...
  template, this variance plane \textit{overestimates} the true variance
  by 30--40\%, resulting in a lower rate of false positives but also
  shallower effective depth.  In the case of smoothing of the
  template, this variance is a 4--5\% \textit{underestimate}, resulting
...
\item We find that the amplitude of DCR shift in the red \texttt{M2.0V}
  stars is under-realized by $\sim 10\%$ compared to expectations.
  \texttt{A0V} stars are over-refracted by $\sim 4\%$ compared to
  expectations.
...
\item The treatment of DCR will likely be a complicated one, and at
  ...
  (which requires registration of input images \textit{and} of the
...
at \\\texttt{/nfs/lsst/home/becker/Winter2014}.  All files not included in
...
found in the \texttt{NOTES} file on GitHub.
...
postfiltering seeing \texttt{1} data (which result in a deconvolution).
...
a positive and negative--going \texttt{Psf} with 6 model terms including
...
In Ticket \#3161 we implemented a fully non--linear joint \texttt{Psf}
fit, using the \texttt{Minuit} package.  This substantially sped up the
...
was performed using script \texttt{python/compareMeasurementTiming.py} in
this repository.  Dipole measurement is currently \textit{not} the tall
...
(or ``trim files'') for \texttt{phoSim}, running these images to create
output \texttt{eImages}, creating \texttt{astrometry.net} index files for
astrometric calibration, and running these images through \texttt{
  processEimage.py}.  This does \textit{not} include the process of
...
\texttt{processCcd.py}, which does (amongst other operations) the
assembly of amp images into CCD images and instrument signature
removal.

The process is valid for \texttt{phoSim} version 3.3.2.

\subsection{Setting up PhoSim}

To download, build, and setup \texttt{phoSim}:
...
Note that you will have to declare the locations of \texttt{cfitsio} and
\texttt{fftw3} by hand, and must enter the full paths above (i.e. not the
string ``\$CFITSIO\_DIR/lib/'' but the path that it resolves to).  The
above assumes that you have a DM stack already, with \texttt{cfitsio} and
\texttt{fftw3} setup.

\subsection{Running \texttt{phosim.py}}

To run \texttt{phoSim}, you need the environment variable
CAT\_SHARE\_DATA declared, as this is where the package looks for its
SEDs.  Example locations are, at NCSA, \texttt{
  /lsst/home/aconnolly/imsim/starSED/}.  You also need to create \texttt{
  work/} and \texttt{out/} directories for the script to use; these must
be made \textit{before} the simulation is run.  A ``trim'' file or
...
control file (named \texttt{clean.params.beta} here) must also be
included, and turns off/on various effects in the sims, such as sky
backgrounds and tracking errors.  A call to the \texttt{phoSim} package
then looks like:
...
where \texttt{-w} defines the work directory (which must already exist),
\texttt{-o} the output directory (which must already exist), \texttt{-c} the
control file, \texttt{-s} the sensors to simulate, and \texttt{-p} the
number of cores to use.


Once the raw amplifier and \texttt{eImages} are created, they need to be
renamed for consistency with the \texttt{obs\_lsstSim} mapper.  The
repository script \texttt{python/rename\_images.py} is used for this,
with an example command that moves the \texttt{eImages} from the \texttt{
  phoSim} output directory to a DM input directory called \texttt{inputs8}:
{\small
\begin{Verbatim}[frame=single]
cd sims8/
foreach i (*.out)
...
A calibration file must be created for \texttt{astrometry.net}, which is
...
Note that the file \texttt{andConfig.py} must be modified to use the
naming convention defined by environment variable \texttt{P} above:
...
Finally, the running of SFM via \texttt{processEimage.py}:
...
where \texttt{-j 20} provides the number of concurrent threads.
...
Important flags here are the W14 configuration file \texttt{
  imageDifferenceConfigGstar.py}, the template visit via \texttt{
  winter2013TemplateId}, and the prefiltering flag via \texttt{
  doPreConvolve}.
...
\begin{itemize}
  \item \texttt{SIM\_MINSOURCE 1} : Only simulate the sensor if 1 or more
    stars land on it
  \item \texttt{SIM\_VISTIME 300.0} : Total exposure time of the visit.
  \item \texttt{SIM\_NSNAP 1} : Request one snap that accounts for all
    \texttt{SIM\_VISTIME} seconds of the simulation.
\end{itemize}

In addition, the control file \texttt{clean.params.beta} was designed to
...
of a random star field populated by stars of SED \texttt{
...
in the $i$--band.  These trim files are stored in the \texttt{sims3/}
directory in this repository.

\subsection{Subtask 2}

We chose spectrum \texttt{km20\_6000.fits\_g30\_6020.gz} for our
reference \texttt{G0V}-star, and replaced 80\% of the objects in the
original trim file with this SED.  We then selected the bluest object
contained in the base catalog, \texttt{kp01\_9750.fits\_g45\_9830.gz},
for every tenth object starting at object \#0, and the most populous
red object in the catalog \texttt{m2.0Full.dat.gz} for every tenth object
starting at object \#5.  These correspond to spectral types \texttt{A0V}
and \texttt{M2.0V}, respectively.
...
using \texttt{python/colors.py}.  Script \texttt{makeNewTrims.py} was used
to make these modifications to the \texttt{sims3/} trim files, yielding
\texttt{sims5/} trim files ($i$--band).  The only modifications made to
the input trim files to simulate images in the $gr$--bands were to
change the \texttt{Opsim\_filter} field to indicate $gri$ = 123; these
are stored in the \texttt{sims5gr/} directory.

\subsection{Subtask 3}

First, to have similar brightness distributions in all data, we make a
SED--dependent \textit{and} passband--dependent magnitude correction to
the trim files used for this task.  These corrections are made in
script \texttt{generateTrimFilesDcr.py}, which ran on the $i$--band trim
files from \texttt{sims5}.
...
both the per-object coordinates and the boresight pointing \texttt{
...
Because the instance catalog inputs to \texttt{phoSim} form an
...
subroutine \\\texttt{makeObsParamsRaDecSky} to generate a self--consistent
set of observation parameters given the Ra and Dec of the field, and
the times of observation established above.

In addition, this script synchronized the values of \texttt{
  Opsim\_rotskypos} and \texttt{Opsim\_rottelpos}.  \texttt{Rotskypos} sets
the orientation of Ra and Dec of the catalog, effectively a rotation
of the Pole away from the Zenith.  When \texttt{rotskypos=0} the Ra
...
rendered in approximately the same orientation.  \texttt{Rottelpos} is
defined as (\texttt{rotskypos}-180+parallactic angle), such that the
...
observation dependent, and was determined using \texttt{
  makeObsParamsRaDecSky}.  Trim files for this run are found in the
\texttt{sims8/} directory of this repository.
...
\begin{figure}[!ht]
  ...
  \caption{\textbf{Refraction}: This figure depicts the amplitude and
    orientation of the refraction vector in the visit \A, \texttt{
      raft=2,2 sensor=1,1 filter=g}, seeing value \texttt{2} data.  The
    vectors point from the unrefracted locations to the realized
    locations in the image.  The SEDs of the sources are indicated
    with colors \texttt{blue}, \texttt{green} and \texttt{red} for \texttt{A0V},
    \texttt{G0V}, and \texttt{M2.0V} respectively.  This figure was created
    using the script \texttt{python/compareDcrFromSims.py}.}
  \label{fig:refractim}
\end{figure}
...
\begin{figure}[!ht]
  ...
  \caption{\textbf{Differential Chromatic Refraction and Difference Image
      Quality, $g$--band}: This set of panels demonstrates the amount
    of refraction (top row), differential refraction with respect to
    the \texttt{G0V} star (middle row), and the quality of the difference
...
\begin{figure}[!ht]
  ...
  \caption{\textbf{Differential Chromatic Refraction and Difference Image
    ...
    $r$--band data.  This figure was created using the script \texttt{
...
\begin{figure}[!ht]
  ...
  \caption{\textbf{Differential Chromatic Refraction and Difference Image
    ...
    $i$--band data.  This figure was created using the script
    \texttt{python/compareDcrFromSims.py}.}
  \label{fig:dcrimi}
\end{figure}
...
\begin{figure}[!ht]
  ...
  \caption{\textbf{Differential Chromatic Refraction and Difference Image
    ...
    was created using the script \texttt{python/compareDcrFromSims.py}.}
  \label{fig:dcrimgpre}
\end{figure}
